    ...
    public class UploadController : Controller
    {
        ...
        private async Task ValidateAsync(string connectionId)
        {
            ...
            if (process.ExitCode != 0)
            {
                ...
                await hubContext.Clients.Client(connectionId).SendAsync("validatedWithErrors", "Der ILICOP hat Fehler in der Datei gefunden.");
            }
            else
            {
                LogInfo("The ilivalidator found no errors in the file. Validation successfull!");
                await hubContext.Clients.Client(connectionId).SendAsync("validatedWithoutErrors", "Der ILICOP hat keine Fehler in der Datei gefunden.");
            }
